...
cwd = os.getcwd()
temp_files = ['.fits','.cat','.log1','.sh','new.param','sub_cube','aa','masked_','m0_']
...
filename='/ibo9000/SDC2/data/sky_full_v2.fits'
...
sextractor_configFile='/ibo9000/SDC2/SHAO/default.sex'
...
#The '*_a.fits.aper.fits' files can be used to see the goodness of the source finding in channel images.
...
pro_arr = np.array_split(np.arange(len(freq)-2),num_process)
...
for k in range(len(freq)):
    ...
    if freq[k]!=float(files6[k].split('_f.cat')[0]):
        logging.info('The frequency %f is not matching with channel catalog file name!'%freq[k])
        continue
...
###################################################
# 5th Step: Making moment-0 map for each source   #
###################################################
...
logging.info('Slicing the full size image cube into sub-cubes around the HI sources.\n')
...
for subcube in subcubes:
    ...
    masked_cube1.write('masked_'+subcube,overwrite=True)
    #Compute the 0th order moment and write to file:
    moment0_map = masked_cube1[1:-2,:,:].moment(order=0, axis=0, how='cube')
    moment0_map.write('m0_'+subcube,overwrite=True)



####################################################
# 6th Step: Source finding from the moment-0 maps  #
####################################################
...
#################################################################################
# 7th Step: Estimating central frequency and line-width from global HI profile  #
#################################################################################
